resource "oci_email_sender" "notifications" {
  ...
  email_address  = format("notifications@%s", var.email_subdomain_name)
...
resource "oci_email_sender" "do_not_reply" {
  compartment_id = oci_identity_compartment.email.id
  email_address  = format("do-not-reply@%s", var.email_subdomain_name)
  # defined_tags = {"Operations.CostCenter"= "42"}
  # freeform_tags = {"Department"= "Finance"}
}

resource "oci_identity_user" "sender" {
  compartment_id = var.tenancy_ocid
  description    = "Robot user to send emails via SMTP and/or API calls."
  name           = "sender"

  email = format("sender@%s", var.email_subdomain_name)
...
resource "oci_identity_user_group_membership" "sender_group_membership" {
